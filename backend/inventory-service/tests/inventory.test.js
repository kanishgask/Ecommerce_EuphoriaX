...
describe('Inventory API', () => {
  ...
  it('GET /api/v1/inventory/:productId returns inventory', async () => {
    ddbDocClient.send.mockResolvedValueOnce({ Item: MOCK_INVENTORY });
    const res = await request(app).get('/api/v1/inventory/prod-1');
    expect(res.status).toBe(200);
    expect(res.body.data.availableStock).toBe(100);
  });

  it('PUT /api/v1/inventory/:productId updates stock', async () => {
    ddbDocClient.send.mockResolvedValueOnce({ Attributes: { ...MOCK_INVENTORY, availableStock: 200 } });
    const res = await request(app).put('/api/v1/inventory/prod-1').send({ quantity: 200 });
    expect(res.status).toBe(200);
  });

  it('PUT /api/v1/inventory/:productId returns 400 on validation error', async () => {
    const res = await request(app).put('/api/v1/inventory/prod-1').send({ quantity: -5 });
    expect(res.status).toBe(400);
  });

  it('POST /api/v1/inventory/reserve reserves inventory', async () => {
    ddbDocClient.send.mockResolvedValueOnce({ Attributes: MOCK_INVENTORY });
    const res = await request(app).post('/api/v1/inventory/reserve').send({
      orderId: 'o-1', items: [{ productId: 'p1', quantity: 5 }],
    });
    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
  });

  it('POST /api/v1/inventory/release releases inventory', async () => {
    ddbDocClient.send.mockResolvedValueOnce({ Attributes: MOCK_INVENTORY });
    const res = await request(app).post('/api/v1/inventory/release').send({
      orderId: 'o-1', items: [{ productId: 'p1', quantity: 5 }],
    });
    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
